...
class LVCBPTOMIntegrator : public Integrator {
public:
    MTS_DECLARE_CLASS()
...
public:
    ...
    LVCBPTOMIntegrator(const Properties &props) : Integrator(props) {
    ...
    LVCBPTOMIntegrator(Stream *stream, InstanceManager *manager)
    ...
    void connectSubpaths(
        ...
        for (int i = 0; i < m_LVCConnectTimes; ++i) {
            ...
            for (int j = 0; j < eyePath.size(); ++j) {
                ...
                BDPTVertex* lightEnd = &m_LVC[choice];
                Spectrum value;
                if(evalContriOM(eyeEnd, lightEnd, scene, value)) {
    ...
    bool evalContriOM(
        ...
        // bool vis = scene->rayIntersect(ray);
        int id = OM::nearestOMindex(d);
        bool vis = roma[id].Visible(eyeEnd->pos + eyeEnd->n*0.5, 
                                    lightEnd->pos + lightEnd->n*0.5); 
        // bool vis=0; 
        if (!vis)
            return false;
...
MTS_IMPLEMENT_CLASS_S(LVCBPTOMIntegrator, false, Integrator)
MTS_EXPORT_PLUGIN(LVCBPTOMIntegrator, "LVCBPT_OM");
MTS_NAMESPACE_END